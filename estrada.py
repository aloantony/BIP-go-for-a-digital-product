...
try:
    while True:
        ...
        if button_pressed:
            ...
            # Send commands to all servos without delays between them
            bus.write(f"#{SERVO_A_ID}D{0}T{MOVE_TIME_MS}\r".encode())            
            bus.write(f"#{SERVO_B_ID}D{0}T{MOVE_TIME_MS}\r".encode())
            bus.write(f"#{SERVO_C_ID}D{0}T{MOVE_TIME_MS}\r".encode())
        ...
        if button_pressed:
            print("Button pressed! Moving servos...")
            sleep(0.5)
            bus.write(f"#{SERVO_B_ID}D{5/2*NUM_PI}T{MOVE_TIME_MS}\r".encode())
            bus.write(f"#{SERVO_C_ID}D{10/2*NUM_PI}T{MOVE_TIME_MS}\r".encode())
            sleep(0.5)
            bus.write(f"#{SERVO_Z_ID}D{2/2*NUM_PI}T{MOVE_TIME_MS}\r".encode())
            sleep(0.5)
            bus.write(f"#{SERVO_A_ID}D{-15/2*NUM_PI}T{MOVE_TIME_MS}\r".encode()) 
            bus.write(f"#{SERVO_B_ID}D{-15/2*NUM_PI}T{MOVE_TIME_MS}\r".encode())
            bus.write(f"#{SERVO_C_ID}D{12/2*NUM_PI}T{MOVE_TIME_MS}\r".encode()) 
            sleep(0.5)
            bus.write(f"#{SERVO_A_ID}D{15/2*NUM_PI}T{MOVE_TIME_MS}\r".encode()) 
            bus.write(f"#{SERVO_B_ID}D{15/2*NUM_PI}T{MOVE_TIME_MS}\r".encode()) 
            # Wait for the movement to complete
