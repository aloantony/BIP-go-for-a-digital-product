...
try:
    while True:
        ...
        if button_pressed:
            ...
            # {servoID}D{angle}T{ms}\r
            bus.write(f"#{SERVO_A_ID}D{ANGLE_A}T{MOVE_TIME_MS}\r".encode())
            sleep(0.5)

            bus.write(f"#{SERVO_B_ID}D{ANGLE_B}T{MOVE_TIME_MS}\r".encode())
            sleep(0.5)

            bus.write(f"#{SERVO_C_ID}D{ANGLE_C}T{MOVE_TIME_MS}\r".encode())
            sleep(0.5)
